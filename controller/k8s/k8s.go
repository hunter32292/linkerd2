...
func waitForCacheSync(syncChecks []cache.InformerSynced, cacheTimeout int32) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Duration(cacheTimeout*int32(time.Second)))
	defer cancel()
